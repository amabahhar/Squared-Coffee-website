...
const Logo: React.FC<LogoProps> = ({ className = '', size = 'md', variant = 'light' }) => {
  ...
  return (
    <div className={`flex flex-col items-center justify-center transition-colors duration-500 ${className}`}>
      ...
      {/* Text part */}
      {size !== 'sm' && (
        <div className={`mt-1.5 flex flex-col items-center gap-0.5 text-center uppercase tracking-widest font-extrabold leading-tight transition-colors duration-500 ${textThemeClasses.primary}`}>
